...
export default async function ClientDashboardPage() {
  ...
  const [invoicesRes, campaignsRes, leadsRes] = await Promise.all([
    ...
    admin.from("leads").select("id,full_name,phone,email,branch,preferred_time,meta_campaign_name,meta_ad_name,status,notes,follow_up_date,created_at").eq("client_id", profile.client_id!).order("created_at", { ascending: false }),
  ]);
